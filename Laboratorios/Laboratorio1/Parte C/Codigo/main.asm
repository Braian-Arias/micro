...
.cseg
Inicio:
    ; Inicialización Stack pointer(paque ande los led
    ldi r16, high(RAMEND)
    out SPH, r16
    ldi r16, low(RAMEND)
    out SPL, r16
...
TIMER1_ISR:
	...
	lds r30, Lutt       ; índice el bit guardado 
    ldi r31, high(lut<<1)
    ldi r17, low(lut<<1)
    add r30, r17
    adc r31, __zero_reg__
    lpm r16, Z         ; ahora sí lee de la tabla con Z+ saltaba uno
	out PORTD, r16             ; enviar a DAC (PORTD)

    ; incrementar índice
    lds r30, Lutt
    inc r30
    cpi r30, LUT_SIZE          ; sí llegamos al final
    brlo No_Inicio
